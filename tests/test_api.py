...
def test_exposed_objects(modulelevel_internal_objects: list[griffe.Object | griffe.Alias]) -> None:
    """All public objects in the internal API are exposed under `efts_io`."""
    not_exposed = [
        obj.path
        for obj in modulelevel_internal_objects
        if obj.name not in efts_io.__all__ or not hasattr(efts_io, obj.name)
    ]
...
def test_single_locations(public_api: griffe.Module) -> None:
    ...
    multiple_locations = {}
    for obj_name in efts_io.__all__:
...
def test_inventory_matches_api(
    ...
    public_api_paths.add("efts_io")
    for item in inventory.values():
        if (
            item.domain == "py"
            and "(" not in item.name
            and (item.name == "efts_io" or item.name.startswith("efts_io."))
        ):
